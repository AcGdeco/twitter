...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
        ...
        //if((!empty($Seguindo[$idusuario]) && $Seguindo[$idusuario] == "Sim") or $idusuario == $id){
            ...
            <p style = "font-weight:bolder;font-size:20px;">'.$nome.' ';

            if(empty($Seguindo[$idusuario]) && $idusuario != $id){
                echo '<span onMouseOver="this.style.opacity=0.5" onMouseOut="this.style.opacity=1" style = "text-align:center;cursor:pointer;font-size:16px;color:white;border:solid 2px white;border-radius:25px;padding:8px;padding-left:25px;padding-right:25px;font-weight:bolder;background-color:#039ff5;" onclick = "seguir('.$idusuario.')" >SEGUIR</span>
                </p>';
            }else if((!empty($Seguindo[$idusuario]) && $Seguindo[$idusuario] == "Sim") && $idusuario != $id){
                echo '<span onMouseOver="this.style.opacity=0.5" onMouseOut="this.style.opacity=1" style = "text-align:center;cursor:pointer;font-size:16px;color:white;border:solid 2px white;border-radius:25px;padding:8px;padding-left:25px;padding-right:25px;font-weight:bolder;background-color:#039ff5;" onclick = "pararSeguir('.$idusuario.')" >SEGUINDO &check;</span>
                </p>';
            }

            echo '<p class="texto-publicacao" style = "word-wrap:break-word;width:100%;" >'.$texto.'</p>
            ...
            echo '<div class="w-form">
                <form id="email-form-2" name="email-form-2" data-name="Email Form 2" class="w-clearfix"><textarea placeholder="..." maxlength="5000" id="field-2-'.$idTweet.'" name="field-2" class="textarea w-input"></textarea><input onclick = "comentar('.$idTweet.')" value="Comentar" data-wait="Please wait..." class="submit-button w-button" style = "width:115px;" ></form>
